...
def retrieve_knowledge(query, context):
    return [[a] for a in retrieve_knowledge_({a:b for a,b in context}, query)]
# def retrieve_knowledge(query, context):
#     ls = []
#     ls2 = []
#     for a in context:
...
def if_fail(text):
    tmp = text.lower()
    if tmp.endswith('given knowledge'):
        return True
    if text=="None":
        return True
...
if __name__=="__main__":
    ...
    parser.add_argument("--dataset_path", type=str, default="tkdrnjs0621/musique_ans_processed_pv2_ner_30", help="model name for evaluation")
    ...
    for row in tqdm(dataset):
        ...
        for question, answer in row["qa_pairs"]:
            ...
            knowledge, ls2 = retrieve_knowledge(question_changed,row["merged_context"])
